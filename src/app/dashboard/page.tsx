      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {withStatus.map((tool) => (
              <tr key={tool.id}>
                <td className="px-4 py-2">{tool.toolCode}</td>
                <td className="px-4 py-2">{tool.name}</td>
                <td className="px-4 py-2">{tool.department}</td>
                <td className="px-4 py-2">{tool.lastCalibrationDate.toLocaleDateString("th-TH")}</td>
                <td className="px-4 py-2">{tool.expiryDate.toLocaleDateString("th-TH")}</td>
